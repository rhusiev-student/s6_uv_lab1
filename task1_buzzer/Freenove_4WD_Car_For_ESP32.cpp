...
void Buzzer_Wiu(int time, int freq) {
  ledcWriteTone(PIN_BUZZER, freq);
  delay(time);
  ledcWriteTone(PIN_BUZZER, 0);
}

void Buzzer_WiiII(int time, int freq_start, int freq_end) {
    int time_diff = 30;
    int times = time / time_diff;
    float diff = float(freq_end - freq_start) / times;

    if (freq_end > freq_start) {
        for (float freq = freq_start; freq <= freq_end; freq += diff) {
            ledcWriteTone(PIN_BUZZER, round(freq));
            delay(time_diff);
        }
    } else {
        for (float freq = freq_start; freq >= freq_end; freq += diff) {
